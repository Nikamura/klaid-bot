...
describe("download-video", () => {
  ...
  describe("downloadVideosFromMessage", () => {
    it("should return early when no URLs found in message", async () => {
      const { downloadVideosFromMessage } = await import("../download-video.js");
      const { ctx, mocks } = createMockContext({ messageText: "Hello world no urls here" });
      const logger = createMockLogger();
      const message = ctx.message;
      assert.ok(message, "Message should exist");

      await downloadVideosFromMessage(message, "Hello world no urls here", logger, ctx, undefined, false);
    ...
    it("should not delete original message when autoDeleteMessage is false", async () => {
      ...
      await downloadVideosFromMessage(message, "Hello no urls", logger, ctx, undefined, false);
    ...
    it("should use whitelist filter when provided", async () => {
      ...
      await downloadVideosFromMessage(message, "Check https://example.com", logger, ctx, ["nomatch.com"], false);
    ...
    it("should not call deleteMessage when no URLs found", async () => {
      ...
      await downloadVideosFromMessage(message, "no urls here", logger, ctx, undefined, true);
